export function buildAlertPrompt(params: {
  ...
  const { anomalyType, timeStr, missingEvents, observedEvents, score } = params;
...
You MUST explicitly state the exact time of the event (e.g., "at ${timeStr}") inside the alert sentence itself.
